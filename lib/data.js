...
// Optional transfer insurance plans. `rate` is a fraction of the transfer value;
// coverage is the full transfer value. There's a small minimum premium.
export const INSURANCE_PLANS = [
  { id: "none", name: "No protection", rate: 0, min: 0, blurb: "Proceed without transfer insurance.", features: [] },
  {
    id: "standard", name: "Standard Protection", rate: 0.005, min: 2,
  ...
  {
    id: "premium", name: "Premium Protection", rate: 0.01, min: 5,
...
// Premium for a plan given a transfer value (rounded to cents, respecting the minimum).
export function insurancePremium(planId, transferValue) {
  const plan = getInsurancePlan(planId);
  if (!plan.rate) return 0;
  return Math.max(plan.min, Math.round(transferValue * plan.rate * 100) / 100);
}
