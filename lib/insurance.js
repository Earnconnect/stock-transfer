...
// Whether the admin currently offers any active insurance plan.
export async function hasActiveInsurancePlans() {
  const count = await prisma.insurancePlan.count({ where: { active: true } });
  return count > 0;
}
